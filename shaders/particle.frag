...
layout(binding = 1) uniform ParticlesUniformBufferObject {
    vec3 Position;
    vec3 HigherCorner;
    vec3 LowerCorner;
} particleUBO;
...
void main() {
    vec4 clipSpacePosition = matricesUBO.projectionMatrix * matricesUBO.viewMatrix * vec4(particleUBO.Position, 1.0);
